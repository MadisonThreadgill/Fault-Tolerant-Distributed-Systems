...
// Lab 2 Added Semaphore for message driver

void ST7735_Message(uint32_t  device, uint32_t  line, char *pt, int32_t value){
  OS_bWait(&LCDFree);// semaphore for LCD
  int print_space=0;
  int extent, col = 19;
  int sign = value < 0 ? -1 : 1;
  line = device == 0 ? line : line + 8;
  value = value * sign;
  
  if(value != 0x8000){
    do {
      ST7735_SetCursor(col,line);
      ST7735_OutChar('0'+value%10);
      value = value/10;
      col--;
    } while( value != 0 );
    if( sign == -1 ){
  ...
  for( col=0; col<extent; col++ ){
    if( print_space || pt[col] == 0 ){
      ST7735_OutChar(' ');
      print_space = 1;
    } else
      ST7735_OutChar( pt[col] );
  }
  OS_bSignal(&LCDFree);
}
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_MAGENTA,ST7735_BLACK, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
