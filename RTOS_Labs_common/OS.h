...
#define TIME_500US  40000 
#define TIME_250US  20000 

#define MAX_THREAD_COUNT    10
#define TCB_NUM             (MAX_THREAD_COUNT)
#define STACK_SIZE          300

#define PD1  (*((volatile uint32_t *)0x40007008))
#define PD0  (*((volatile uint32_t *)0x40007004))
    
// TCB struct initialization
// this is also a node for the doubly linked list structure
struct TCB{
   struct TCB *next_tcb; 	 // pointer for next tcb
   int32_t curr_stack_ptr;   // stack pointer for this thread
   uint8_t id; 			     // thread id
   uint8_t priority; 		 // priority of thread 
   int32_t sleep_state;      // if thread is asleep, sleep_state > 0
   int32_t blocked_state;    // if thread is blocked, blocked_state > 0
};
...
void OS_Record_Jitter(uint8_t Channel, uint32_t period);
